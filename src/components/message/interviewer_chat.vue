<template>
	<div class="interview_chat w-100">
		...
		<div class="info-items" @click="hiddenIcon">
			<!-- <ul class="info-item bg-white" v-for="(item,i) in interviewerList" :key="i"> -->
			<ul class="info-item bg-white" ref="itemsInfos">
				<li class="item1">
					<span class="fm-1 fc-dark fw-6">{{interviewerData.title}}</span>
					<span class="fm-1 fc-blue fw-6">{{interviewerData.salary}}</span>
				</li>
				<li class="item2">
					<span class="fc-dark fs-3 fw-6">{{interviewerData.info}}</span>
				</li>
				<li class="item3">
					<span class="fc-gray fw-6 fs-3">{{interviewerData.addr}}</span>
				</li>
				<li class="item5">
					<span class="fc-gray fw-6 fs-3">{{interviewerData.experience}}</span>
					<span class="fc-gray fw-6 fs-3">{{interviewerData.education}}</span>
				</li>
				<li class="item4" v-if="isBtnCode">
					<a href="javascript:;" @click="updatedStatus(4)">
						<span class="fm-1">发简历</span>
					</a>
			...
			<!-- 单条消息 -->
			<ul class="msg-list mt-5" id="elm0" :style="{height:(curHeight-headerHeight-itemInfosHeight-100)+'px'}">
		...
		<div class="eiej-model" ref="eiejElement" :style="eiejIndex">
			...
			<div class="recorder-model" v-show="modeState == 1">
				...
				<a href="javascript:;" v-if="blobUrl!=null">
					<audio :src="blobUrl" id="Audios2" preload></audio>
				</a>
...
export default {
	...
	methods: {
					...
					switch (status) {
						...
						case 4: //已发送简历
							if (this.baseUserId == this.postUserId) {
								...
								this.isPayCode = false;
								this.pageInfo.info = "同意，邀请面试";
								// this._msg("同意，邀请面试", 800);
							} else {
								this.isBtnCode = false;
								this.isAgreeCode = false;
								this.isPayCode = false;
								this.pageInfo.info = "已发送简历等待对方同意！！";
								// this._msg("已发送简历等待对方同意！！", 800);
							}
							break;
						case 5: //已约面试
							if (this.baseUserId == this.postUserId) {
		...
		topAuto() { //聊天信息在优先展示
			var ele = document.querySelector('#elm0')
