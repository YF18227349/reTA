<template>
	<div class="company_auth w-100">
		<!-- <header class="company_auth_header bg-gray w-100 fc-dark">
		...
		<div class="company_auth_group w-100">
			<ul class="auth_id_list">
				<li>
					<yd-cell-item>
						<span slot="left" class="auth_id_title">请拍照并上传营业执照</span>
					</yd-cell-item>
				</li>
				...
				<li class="mt-4">
					<yd-cell-item>
						<span slot="left" class="auth_id_title">请拍摄并上传企业法人的身份证照片</span>
					</yd-cell-item>
				</li>
				<li class="shot_group w-100">
					<a href="javascript:;" class="shot_id_item p-r">
				...
				<li>
					<span slot="left" class="auth_id_title">拍摄营业执照及身份证要求:</span>
				</li>
				<li>
					<span slot="left" class="fc-gray fs-3 fw-5">所在公司的有效营业执照;</span>
				</li>
				<li>
					<span slot="left" class="fc-gray fs-3 fw-5">公司法人的有效二代身份证;</span>
				</li>
				<li>
...
export default {
	...
	mounted() {
		...
		$(".company_auth_group").css("height", `${h * 0.93}px`);
	},
	...
	methods: {
		...
		getAuthInfo(token, type) {
			...
			this.photo_list[0].photo = value.openimg;
			this.photo_list[1].photo = value.idCard;
			this.photo_list[2].photo = value.idCards;
		...
		release() {
			var token = this.pageInfo.token;
			var url = this.Global.baseURL + "/Approval-addEnterprise.jsp";
			var openimg = this.upload_file_list[0];
			if (!openimg) {
				openimg = this.regSplic(this.photo_list[0].photo, 1);
			};
			var idCard = this.upload_file_list[1];
			if (!idCard) {
				idCard = this.regSplic(this.photo_list[1].photo, 1);
			};
			var idCards = this.upload_file_list[2];
			if (!idCards) {
