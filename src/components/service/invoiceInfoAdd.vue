		...
		<ul class="add_invoice_info_tabs bg-white">
			<li v-for="(item,i) in pageInfo.tabList" :key="i" class="tab_item">
				<a href="javascript:;" class="item" @click="select_tab(i)">
					<span class="item_code" :class="pageInfo.tabCode==i?'selectTab':'unSelectTab'">{{item.title}}</span>
				</a>
			</li>
		</ul>
			...
			<ul class="add_info_list w-100">
				<li class="p-r">
					<div class="add_info_item">
						<span>名称</span>
						<input class="item_input" type="text" v-model="name" :placeholder="pageInfo.tabCode==0? '请输入单位名称（必填）':'请输入姓名（必填）'">
					</div>
				...
				<li class="p-r">
					<div class="add_info_item">
						...
						<input class="item_input" type="text" v-model="code_id" :placeholder="pageInfo.tabCode==0?'15-20位数字和字母（必填）':'请输入纳税人识别账号（必填）'">
					</div>
				...
				<li class="p-r">
					<div class="add_info_item">
						<span>地址</span>
						<input type="text" class="item_input" v-model="addr" :placeholder="pageInfo.tabCode==0?'请输入单位地址信息':'请输入个人地址信息'">
					</div>
					<a href="javascript:;" class="clearBtn p-a" :class="addrShow?'bg-lightdark':'bg-white'" @click="addr=''">
						<span class="fw-7 fc-white iconfont icon-error"></span>
					</a>
				</li>
				<li class="p-r">
					<div class="add_info_item">
						<span>电话</span>
						<input type="tel" class="item_input" v-model="telNum" maxlength="11" placeholder="请输入电话号码">
					</div>
				...
				<li class="p-r">
					<div class="add_info_item">
						<span>开户银行</span>
						<input type="text" class="item_input" v-model="bank_name" maxlength="11" placeholder="请输入开户地址">
					</div>
					<a href="javascript:;" class="clearBtn p-a" :class="bankNameShow?'bg-lightdark':'bg-white'" @click="bank_name=''">
						<span class="fw-7 fc-white iconfont icon-error"></span>
					</a>
				</li>
				<li class="p-r">
					<div class="add_info_item">
						<span>银行账户</span>
						<input type="number" class="item_input" v-model="bank_code" maxlength="11" placeholder="请输入银行账户">
					</div>
					<a href="javascript:;" class="clearBtn p-a" :class="bankCodeShow?'bg-lightdark':'bg-white'" @click="bank_code=''">
						<span class="fw-7 fc-white iconfont icon-error"></span>
					</a>
				</li>
			</ul>
...
export default {
	...
	methods: {
		init() {
			...
			if (id > 0) {
				...
				this.pageInfo.header_title = "编辑发票信息";
				this.__initAction("Invoice-show", param, (res, s) => {
					if (s == 1) {
						var data = res.data[0];
						this.select_tab(data.invoiceType - 1);
						this.name = data.invoiceTitle;
						this.code_id = data.invoiceTax;
						this.addr = data.invoiceAddress;
						this.telNum = data.invoicePhone;
						this.bank_name = data.invoiceBank;
						this.bank_code = data.invoiceAccount;
					}
				});
			} else {
				this.pageInfo.header_title = "添加发票信息";
				this.select_tab(1);
				if (this.deBug()) {
					this.name = ''; // 测试单位名称
					this.code_id = ''; // 612601199503200916
					this.addr = ''; // 测试单位地址
					this.telNum = ''; // 13366666666
					this.bank_name = ''; // 测试开户地址
					this.bank_code = ''; // 6222023700040128813
				}
			}
		...
		save_info() {
			...
			this.loading("正在保存请稍后...");
			var id = this.getParams().id;
			var param = info;
			param.token = this.getToken();
			if (id > 0) {
				param.id = id;
				var ajaxLink = "Invoice-edit";
			} else {
				var ajaxLink = "Invoice-add";
				this.pageInfo.header_title = "添加发票信息";
			}
			this.__initAction(ajaxLink, param, (res, s) => {
				console.log(res,s)
				if (s == 1) {
					this.setCache("InfoData", JSON.stringify(info));
					this._msg(res.info, 1000, "success");
					this.__Sleep(e => {
						this.$router.back(-1);
					}, 1000);
				}
	...
	watch: {
		...
		bank_name() {
			...
			// this.pageInfo.tabList[this.pageInfo.tabCode].data.bank_name = this.bank_name;
		},
		bank_code() {
			if (this.bank_code) {
			...
			// this.pageInfo.tabList[this.pageInfo.tabCode].data.bank_code = this.bank_code;
		}
		// selcetTabCode() {
		//   var list = this.pageInfo.tabList;
		//   var i = this.pageInfo.tabCode;
